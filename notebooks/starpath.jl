...
begin
	if model_function == "Linear model"
		...
		md"""
		👉 **Choose the parameters** of the **Linear** model function:  ``y = `` $(Lin_1)   ``\cdot x `` $(Lin_2)"""
		
	elseif model_function == "Quadratic model"
		md"""
		👉 **Choose the parameters** of the **Quadratic** model function:  ``y = `` $(Quad_1)  ``\cdot x^2 `` $(Quad_2)  ``\cdot x `` $(Quad_3)
		"""
		
	elseif model_function == "Cubic model"
		md"""
👉 **Choose the parameters** of the **Cubic** model function:  ``y = `` $(Cubic_1) ``\cdot x^3`` $(Cubic_2) ``\cdot x^2 `` $(Cubic_3) ``\cdot x `` $(Cubic_4)
"""
	end
...
begin
	...
	hint( md"""**The optimal parameters** according to orthodox regression can be obtained by solving the matrix equation ``M(\textrm{\textbf{x}}) \cdot \vec a = \textbf{h}_{\measuredangle}``
		
 $(sr.(M(x)\height, 4))

 		

In this case we assume the uncertainties ``\sigma_\nu`` of the height angle ``h_{\measuredangle}`` to be all the same for all data points.
