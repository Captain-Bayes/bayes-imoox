...
But the normalization and the marginalization rule are slightly diﬀerent, as we do have integrals instead of sums. %8_1
 \begin{figure}[H]
	\centering
...
$p(x)=x^2$ and $y(x)=tan(x)$. Then, the pdf of y is $p(y)=arctan(y)^2\frac{1}{1+y^2}$\\
}}
\\
...
We can define these endpoints by the angles $\phi_1$ and $\phi_2$ formed by the \textit{radial lines to the endpoints with the x-axis}. These are two independent variables that uniquely deﬁne the line.\\%8_4
 \begin{figure}[H]
	\centering
...
which implies $\phi < \frac{\pi}{6}$. Hence the desired probability is $\frac 13$.\\%8_6
 \begin{figure}[H]
	\centering
...
Instead of the cartesian coordinates x and y we use spherical coordinates r and $\phi$. The transformation law yields for the correctly normalized pdf the following expression. %8_7
 \begin{figure}[H]
	\centering
...
c) We need a default model $p_0$ in the entropy $S$ i the continuous case, since variables are scale invariant.
}}
\\
...
This distribution is depicted in the ﬁgure.\\%8_15
 \begin{figure}[H]
	\centering
